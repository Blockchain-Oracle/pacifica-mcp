#!/usr/bin/env node
/**
 * @pacifica-dev/mcp — Pacifica MCP stdio server entry point.
 *
 * This package is a THIN wrapper: all tool implementations live in
 * `@pacifica-dev/cli` (the standalone CLI package), which exports a
 ...
 * (`@pacifica-dev/mcp` → `pacifica-mcp`) is the only clean way to make
